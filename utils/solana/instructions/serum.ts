...
export function decodeNewOrderV3(ix: TransactionInstruction): NewOrderV3 {
    return {
        ...
        data: create(
            decodeInstruction(ix.data).newOrderV3,
            NewOrderV3Instruction
        ),
        programId: ix.programId,
    };
}
...
export function isSerumInstruction(instruction: TransactionInstruction) {
    return (
        ...
        MARKETS.some(
            (market) =>
                market.programId &&
                market.programId.equals(instruction.programId)
        )
    );
}
...
// [QUICK_WALELT] added export here
export const SERUM_CODE_LOOKUP: { [key: number]: string } = {
    0: 'Initialize Market',
    1: 'New Order',
    10: 'New Order v3',
    11: 'Cancel Order v2',
