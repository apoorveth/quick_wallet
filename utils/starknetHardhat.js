...
function isArray(type) {
    return (
        type.startsWith(ARRAY_TYPE_PREFIX) && type.endsWith(ARRAY_TYPE_SUFFIX)
    );
}
    ...
    for (let i = 0; i < inputSpecs.length; ++i) {
        ...
        if (COMMON_NUMERIC_TYPES.includes(inputSpec.type)) {
            ...
            if (isNumeric(currentValue)) {
                ...
                !isCairo1 &&
                inputSpec.name.endsWith(LEN_SUFFIX_DEPRECATED)
            ) {
                // [QUICK_WALLET] adding this line to add length manually
                adapted.push(toNumericString(currentValue));

                const nextSpec = inputSpecs[i + 1];
                const arrayName = inputSpec.name.slice(
                    0,
                    -LEN_SUFFIX_DEPRECATED.length
                );
                if (
                    nextSpec &&
            ...
            const value = validateAndConvertBooleanInput(
                currentValue,
                errorMsg
            );
            adapted.push(value);
...
export function adaptOutputUtil(rawResult, outputSpecs, abi) {
    ...
    for (const outputSpec of outputSpecs) {
        ...
        if (COMMON_NUMERIC_TYPES.includes(outputSpec.type)) {
            ...
            adapted[outputNameOrDefault(outputSpec.name)] =
                convertOutputToBoolean(currentValue);
            resultIndex++;
        } else if (isU256(outputSpec.type)) {
            const lo = currentValue;
            const hi = result[++resultIndex];
            adapted[outputNameOrDefault(outputSpec.name)] = convertOutputToU256(
                lo,
                hi
            );
            resultIndex++;
...
function generateComplexOutput(raw, rawIndex, type, abi) {
    ...
    if (isU256(type)) {
        return {
            generatedComplex: convertOutputToU256(
                raw[rawIndex],
                raw[rawIndex + 1]
            ),
